...
def generate_monthly_report(
    ...
    report_path.write_text(
        render_report(rows, mode=mode, recent_days=recent_days, generated_at=generated_at, used_sample=used_sample),
        encoding="utf-8",
    )
...
def render_report(
    ...
    if mode == "internal":
        return render_internal_report(rows, recent_days, generated_at, used_sample)
    return render_customer_report(rows, recent_days, generated_at, used_sample)


def render_customer_report(
    rows: list[dict[str, str]], recent_days: int, generated_at: datetime, used_sample: bool
) -> str:
    lines = [
        "# 月次脆弱性レポート（顧客向け）",
        "",
        f"- 作成日時: {generated_at.astimezone().strftime('%Y-%m-%d %H:%M %Z')}",
        f"- 対象期間: 直近 {recent_days} 日",
        f"- 掲載件数: {len(rows)} 件",
        f"- データ: {'サンプルCSV' if used_sample else 'CISA KEV / FIRST EPSS'}",
        "",
        "## 1. サマリ",
        "",
        "本レポートは、悪用が確認されている脆弱性と、今後悪用される可能性が相対的に高い脆弱性を確認するためのものです。まずは対象製品の利用有無、外部公開有無、適用済みパッチの確認を推奨します。",
        "",
        "## 2. 今回確認すべきポイント",
        "",
...
def render_internal_report(
    ...
    lines = [
        ...
        "## 1. 今月の傾向",
        "",
        f"- KEV該当: {sum(1 for row in rows if row.get('kev', '').lower() == 'true')} 件",
        ...
        "- EPSSは今後30日程度の悪用可能性の推定であり、単独のリスクスコアとして扱わない。",
